...
async def get_all_user_admin(session=Depends(get_conn_db)):
    ...
    """
    return await crud.users.get_all_users(session)


@router.get(
    "/{user_id}", response_model=shs.ResponseUser, dependencies=[role_required("admin")]
)
async def get_info_user(user_id: int, session=Depends(get_conn_db)):
    ...
    headers Autorization: Bearer <token>
    """
    result = await crud.users.get_user_by_id(user_id, session)
    if not result:
        raise HTTPException(
...
async def block_user(user_id: int, session=Depends(get_conn_db)):
    ...
    headers Autorization: Bearer <token>
    """
    result = await crud.users.set_active_user(user_id, session)
    if not result:
        raise HTTPException(
            status_code=status.HTTP_404_NOT_FOUND, detail="user not found"
        )
